...
- **Always declared.** The tool spec lives outside `RESEARCH_TOOLS`; `toOpenAiTools()` and `toGeminiToolDeclarations()` always include it, so the prompt always mentions it.
