...
describe('handleReview', () => {
  it('on calls sendCommand with review action on and prints ON', async () => {
    const srv = await startServer((_req, res) => {
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ ok: true, settings: { review: { enabled: true } } }));
    });
    const api = new ApiClient(srv.port);
    const { handleReview } = await import('../review');
    const { stdout } = await capture(() => handleReview(['on'], api));
    expect(stdout).toContain('ON');
    srv.close();
  });

  it('off calls sendCommand with review action off and prints OFF', async () => {
    const srv = await startServer((_req, res) => {
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ ok: true, settings: { review: { enabled: false } } }));
    });
    const api = new ApiClient(srv.port);
    const { handleReview } = await import('../review');
    const { stdout } = await capture(() => handleReview(['off'], api));
  ...
  it('no args shows status from getSettings and usage', async () => {
    const srv = await startServer((_req, res) => {
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ review: { enabled: true } }));
    });
    const api = new ApiClient(srv.port);
    const { handleReview } = await import('../review');
    const { stdout } = await capture(() => handleReview([], api));
    expect(stdout).toContain('ON');
